...
const chunkAll = (text: string, limit = 60): Chunk[] => {
...
describe('T035 streaming equals batch', () => {
  const feed = (text: string, size: number, limit = 60): Chunk[] => {
    const c = new Chunker({ maxUnits: limit })
    const out: Chunk[] = []
    for (let i = 0; i < text.length; i += size) out.push(...c.addText(text.slice(i, i + size)))
    out.push(...c.finish())
    return out
  }
...
describe('T032 abbreviation guard', () => {
  const oneChunk = (t: string) => chunkAll(t, 200)
...
describe('T038 the size limit is injected', () => {
  it('accepts a custom unit counter (tokens rather than characters)', () => {
    ...
    for (const ch of chunks) expect(Math.ceil(ch.text.length / 4)).toBeLessThanOrEqual(5 + 1)
  })

  it('defaults to counting characters', () => {
    const chunks = chunkAll('A short one. Another short one.', 16)
    for (const ch of chunks) expect(ch.text.length).toBeLessThanOrEqual(17)
  })
})
