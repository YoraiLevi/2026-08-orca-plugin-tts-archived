...
| 10 | `SpeechService` outcome → the announcement channel | every outcome that is a LOSS gets a sentence; every outcome that is a CONTROL does not | — | **asked, reasoned, NOT tested.** `'empty'` → `unspeakable`, `'synthesis-failed'` → its own sentence, and `'cancelled'`/`'skipped'`/`'superseded'` are deliberately silent because each is something the listener just did. Judged correct by reading, which is exactly the method this section exists to distrust. **Recorded as the weakest row here.** |
...
- **Seam 10 is reasoned, not tested.** It is the one row in this section produced by the method the
  section exists to replace, and it is labelled so.
