...
<!-- why: PINNED, 2026-08-21, by the author's standing decision. This document is a RECORD OF AN
     INVESTIGATION, not a live claim about today's code. Its `path:line` citations point at the
     tree as it stood when the analysis was made (added at eb850bf); they are deliberately NOT
     maintained against HEAD.

     The reason is not cost. 66 of its citations read stale, and a sample of three split
     2 REMEDIED / 1 DRIFTED: TT1 and TT2 quote bare `catch` blocks that have since been FIXED —
     `huddle/index.ts` now separates ENOENT from everything else, and returns `format: 'unknown'`
     so the listener hears "cannot read this transcript". TT3's `catch { return null }` merely
     moved, 31 -> 49. Re-pointing TT1 would land its citation on the REMEDY while the sentence
     beside it describes the defect. A stale pointer is a navigation problem; a re-pointed pointer
     on a remedied defect is a FALSE CLAIM, and no tool can tell the two apart — only a human
     reading each row against today's code.

     To find what a row says about the CURRENT tree, read the row and check it yourself. The
     alternative — reconciling all 66 row by row, which turns this document from a record of what
     was found into a status report requiring maintenance forever — is catalogued as a GitHub
...
| 17 | a **test** → the machine's **scheduler** | *"has the async work finished?"* must be answered by the work, not by a duration | **SC-17** | **OPEN** — R12-01 |
| 15 | the **filesystem** (`fs.watch`) → the transcript tailer | a watcher that dies must be detectable; the OS's own signal must not be discarded | **SC-15** | **OPEN** — R11-01 |
| 16 | the **source manifest** → the **shipped manifest** → ORCA's host | the two copies of the contract must agree, and the checks must not be split across them | **SC-16** | **OPEN** — R11-02 |
| 14 | the modules the Lab imports → **the resolver that ships** | every module must load under **plain node**, not only under vitest's resolver. **Three** resolvers disagree — vitest, plain node, `tsc` — and no specifier satisfies all three | **SC-14** | **OPEN, blocking** — R10-06 |
| 4 | `Chunker` → the engine's **argument vector**, all three platforms | the constructed argv delivers the text as an **operand**, never an option | **SC-3 (v2)** | **closed, round 10** — and it closes 22.5's platform gap for this row |
...
| **SC-3 (v2)** | **REWRITTEN, round 10.** The v1 row asserted a property of the CHUNK and could never pass, so it went permanently amber once the defect was fixed — P32 inside the instrument. v2 asserts the property of the **argument vector the provider builds**: over eight hostile rows × three platforms × three Linux rungs, the text is delivered as an **operand**, never an option. Windows takes a different route to the same property and gets a different assertion | `packages/providers/src/seams/argv-seam.test.ts` | a **permanent** control — the same assertion run against a deliberately broken builder, required to fail; giving that builder a `--` makes the control go red |
| **R10-07** | the three exported builders, created *"so ONE hostile corpus can be run through ALL THREE"*, still disagree: `linuxCommand` neutralises `[[` and `darwinCommand`/`win32Command` do not. Production is safe; the **exported seam** is not, and a direct caller gets NM14 | same file | `it.fails` — nothing audible is wrong yet |
...
| **SC-14** | vitest resolves a `.js` specifier onto a `.ts` file; **plain node does not**, and plain node is what `pnpm voice-lab` runs. So `chunker/index.ts`'s `import … from '../speakable.js'` gives **21 green tests and a Lab that cannot start**. Two resolvers, one of which is the only one that ships | `scripts/seam-stage-identity.test.mjs` | **red in the wild right now**, with two controls that pass — a clean module loads, and a missing module is detected |
| **SC-15** | an **atomic rename-replace** kills the watch: `fs.watch` emits `rename`, **never `error`**, and nothing fires again. The tailer's `'error'` subscription cannot see it, and the callback discards `eventType` in its argument list — so one session goes permanently quiet while every other works. `006` TT13 records this as closed; it is closed against the wrong channel | `packages/plugin/src/seams/tailer-seam.test.ts` | the probe **is** the measurement, with a control — appends with no replace must keep firing |
| **SC-16** | the manifest exists **twice** and `build.mjs` merely `cp`s one onto the other, so the shipped copy is stale whenever a rebuild is not committed. The checks are split — the unit tests read **source**, CI's `smoke-activate` reads **dist** — so neither compares them, and **`read-aloud.self-test`**, the instrument for section 19's rank-one undetectable, is declared in source and **absent from the shipped manifest** | same file | **red at committed `HEAD` in a detached worktree**: 8 commands against 9 |
| **SC-17** | **nine test files wait on a fixed sleep**, which is a prediction about how fast this machine is — and the machine is the one part of the system nobody controls. Two clean worktrees minutes apart gave **657/658 and 653/658**, four of the five reds being load artefacts. **A suite count today means "the machine was quiet", not "the code is correct"** — and every count in every document in this repo rests on that suite | `packages/plugin/src/seams/tailer-seam.test.ts` | a **control** that must fire on the pattern and must NOT fire on the condition-based fix, so a detector flagging the fix as the defect cannot deliver a verdict |
...
### 22.4d The inventory is complete — five for five

Rounds 9 to 11 opened every adjacent pair in the system. Recorded here because the *pattern* is the
result, not any one row:

| Inventory | Opened | Yielded on first look |
|---|---|---|
| the audio path | round 9 | **yes** — SC-1 … SC-10 |
| the transcript decoder | round 10 | **yes** — SC-12, SC-12b, plus a census that invalidated a test corpus |
| the normalizer's build-time contract | round 10, **by accident** | **yes** — SC-13, SC-14 |
| the transcript tailer | round 11 | **yes** — SC-15 |
| the adapter and manifest | round 11 | **yes** — SC-16 |

**Five for five, including the one where most questions came back empty.** No inventory has been
opened and found clean. That is the strongest single argument this document can make for why
section 22 had to exist: the 130 rows above it are organised by component and none of these sixteen
...
- **~~The transcript TAILER and the ADAPTER have no seam rows.~~ CLOSED in round 11** — SC-15 and
  SC-16. **Every adjacent pair in the system now has a row**; see 22.4d.
- **~~Windows and Linux are reasoned from source.~~ CLOSED FOR SC-3 in round 10.** J24 made the argv
  builders pure and exported, so all three platform branches are checkable on one machine — the
  claim moved to something a pure function can answer. **SC-10 keeps the split** and is the further
  along of the remaining ones: its platform half has a probe written and CI-wired (J25).
