...
| **C3** | **MET** `[measured-here]` | Re-measured 2026-08-21 19:43 at `e9162f0`, **clean detached worktree, load average 5.27**, headless and muted. Four arms, all passing the 2,000 ms gate: **cold `short.md` p95 1,690 ms** (n=20) · **cold longest fixture p95 1,327 ms** (n=5) · **arrow speak-on-change p95 1,466 ms** (n=20) · **warm replay p95 41 ms** (n=20). Against the previous **3,401 ms**. P39's warm collapse is genuinely repaired, not masked: the `cachehit.zero-network` control reports **0 `POST /speak` across 20 warm trials**. The earlier refusal to measure was right — the same harness on a loaded machine produced a 71 s control reply. |
