...
| Tests | **727 / 727, 31 files** at `3f31987` `[measured-here]`, clean detached worktree, **load average 5.27**. Take this number nowhere else: the same suite gave 653/658 and 657/658 within minutes on a loaded machine, read 139 stale instead of 85 while a peer had a file open, and once returned 13-of-20 failures because the run straddled a peer's commit. **A count means something only from a detached worktree, at a named commit, at a named load** — all three mechanisms are recorded in `PITFALLS.md` P40. |
